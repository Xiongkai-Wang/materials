...
//    }
    ...
    public String uploadFileFolder(HttpServletRequest request, HttpSession session) {
        ...
        if(!item.matches("[A-Z][a-z]*[1-9]*[A-Z]*[a-z]*[1-9]*[A-Z]*[a-z]*[1-9]*"))
            return "foreWeb/uploadFileError";
        if (type.equals("singleAll")){
            item = FileUtil.obtain(files);
            if (item.equals("data")){
                return "foreWeb/uploadFileError";
            }
            method = "上传文件（全部替换）";
    ...
    public ModelAndView crystalStructure2(@RequestParam(value = "id") String id,@RequestParam(value = "type",required = false,defaultValue = "PBE")String CalType) {
...
//        AlNiZr(2).cif
    ...
    public ModelAndView electronicStructure(@RequestParam(value = "id") String id,@RequestParam(value = "type",required = false,defaultValue = "PBE")String CalType) {
        ...
        String pathYuanSu = "D:\\data\\"+materialName+"\\"+CalType+"\\electronic properties\\" + materialName;
...
//        }
    ...
    public ModelAndView phonon(@RequestParam(value = "id") String id,@RequestParam(value = "type",required = false,defaultValue = "PBE")String CalType) {
        ...
        String materialName = upLoadMaterialMapper.selectByPrimaryKey(id).getMaterialname();
        //Phonon Dispersion路径
        String path = "D:\\data\\"+materialName+"\\"+CalType+"\\phonon spectrum\\"+materialName+" Phonon Dispersion.csv";
        //读取路径下数据
        List<float[][]> data_phonon = chartService.getPhononDisp(path);
        //Phonon DOS 路径
        String pathDOS = "D:\\data\\"+materialName+"\\"+CalType+"\\phonon spectrum\\"+materialName+" Phonon DOS.csv";
        // 读取
    ...
    public ModelAndView elasticity(@RequestParam(value = "id") String id,@RequestParam(value = "type",required = false,defaultValue = "PBE")String CalType) {
        ...
        try {
            ...
            ArrayList<Float[]> young = new ArrayList<>();
            reader2.readLine(); // first line
            while (reader2.readLine() != null) {
                String[] temp = reader2.readLine().split("\\s+");
                Float[] row = new Float[3];
                for (int j=0; j<3; j++) {
                    Float num = Float.parseFloat(temp[j]);
                    row[j] = num;
...
//    }
    ...
    public ModelAndView bandDensity(@RequestParam(value = "id") String id,@RequestParam(value = "type",required = false,defaultValue = "PBE")String CalType) {
    ...
    public ModelAndView generalDensity(@RequestParam(value = "id") String id,@RequestParam(value = "type",required = false,defaultValue = "PBE")String CalType) {
        ModelAndView mv = new ModelAndView("/foreWeb/generalDensity");
        //此处更新路径
        String materialName = upLoadMaterialMapper.selectByPrimaryKey(id).getMaterialname();
        String Path = "D:\\data\\"+materialName+"\\"+CalType+"\\electronic properties\\"+materialName+" DOS.csv";
    ...
    public ModelAndView partitionDensity(@RequestParam(value = "id") String id,@RequestParam(value = "type",required = false,defaultValue = "PBE")String CalType) {
        ModelAndView mv = new ModelAndView("/foreWeb/partitionDensity");
        String materialName = upLoadMaterialMapper.selectByPrimaryKey(id).getMaterialname();
        String Path = "D:\\data\\"+materialName+"\\"+CalType+"\\electronic properties\\"+materialName+" PDOS.csv";
        // 分态密度图
